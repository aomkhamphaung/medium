...
<body>
                        ...
                        @auth
                            @if (Auth::check())
                                <a class="nav-link text-dark" aria-current="page"
                                    href="{{ route('posts.create') }}">{{__('message.write')}}</a>
                            @else
                                <a class="nav-link text-dark" aria-current="page" href="{{ route('login-form') }}">{{__('message.write')}}</a>
                            @endif
                        @endauth
                    </li>
                    @guest
                    <li class="nav-item">
                        <a class="nav-link text-dark" href="{{ route('login-form') }}">{{__('message.login')}}</a>
                    </li>
                    <a href="{{ route('register') }}" class="get-btn btn navbar-btn rounded-pill bg-dark text-white">{{__('message.get_started')}}</a>
                    @endguest

                    @auth
                    <a href="{{ route('posts.index') }}" class="get-btn btn navbar-btn rounded-pill bg-dark text-white">{{__('message.lists')}}</a>
                    @endauth

                    <li class="nav-item dropdown ms-2">
                        <select name="language" class="rounded-pill bg-dark text-white get-btn btn navbar-btn" id="language_option">
    ...
    <div class="container-fluid bg-warning border-bottom border-dark">
        <div class="container banner">
            ...
            @auth
                <a href="{{ route('posts.index') }}" class="banner-btn btn rounded-pill bg-dark text-white btn-text mb-2">
                    Start reading
                </a>
            @else
                <a href="{{ route('register') }}" class="banner-btn btn rounded-pill bg-dark text-white btn-text mb-2">
                    Start reading
                </a>
            @endauth
                ...
                @foreach ($posts as $post)
                    <div class="row">
                        <div class="col-8 mt-3">
                            <div class="d-flex justify-content-start">
                                ...
                                <div class="px-2">
                                    ...
                                    <span class="blog-date">{{ $post->created_at->diffForHumans() }} · 9 min read
                                        ·</span>
                                    <span class="cat"><a href="lists.html">{{ $post->category }}</a></span>
                                    ...
                                    @auth
                                        @if (Auth::check())
                                            @if (Auth::user()->id === $post->user->id)
                                                <a class="content-btn" rel="noopener follow"
                                                    href="{{ route('posts.show', $post->id) }}"><svg width="25"
                                        ...
                                        @else
                                            <a class="content-btn" rel="noopener follow"
                                                href="{{ route('login-form') }}"><svg width="25" height="25"
                                                    viewBox="0 0 25 25" fill="none" class="mf">
                                                    <path
                                                        d="M18 2.5a.5.5 0 0 1 1 0V5h2.5a.5.5 0 0 1 0 1H19v2.5a.5.5 0 1 1-1 0V6h-2.5a.5.5 0 0 1 0-1H18V2.5zM7 7a1 1 0 0 1 1-1h3.5a.5.5 0 0 0 0-1H8a2 2 0 0 0-2 2v14a.5.5 0 0 0 .8.4l5.7-4.4 5.7 4.4a.5.5 0 0 0 .8-.4v-8.5a.5.5 0 0 0-1 0v7.48l-5.2-4a.5.5 0 0 0-.6 0l-5.2 4V7z"
                                                        fill="#292929"></path>
                                                </svg>
                        ...
                        <div class="col-4 mt-3">
                            @if (Auth::check())
                            <a href="{{ route('posts.show', $post->id) }}" title="View Detail"><img
                                src="{{ asset('storage/images/' . $post->image) }}" class="img" /></a>
                            @else
                            <a href="{{ route('login-form') }}" title="View Detail"><img
                                src="{{ asset('storage/images/' . $post->image) }}" class="img" /></a>
                            @endif
                        </div>
                    </div>
